...
import { coordaState, coordbState, intensityState } from "../store";

import Point from "../script/Point";
import { css } from "@emotion/css";
import BarSlide from "../script/BarSlide";

export const IntensitySlicing = () => {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  let ctx: CanvasRenderingContext2D;

  const [coorda, setCoorda] = useRecoilState(coordaState);
  const [coordb, setCoordb] = useRecoilState(coordbState);
  const [intensity, setIntensity] = useRecoilState(intensityState);

  const pf = new Point(0, 10);
  const pl = new Point(255, 10);
